...
i2c = I2C(3) # GPIO23 = SDA, GPIO24 = SCL for this interface
...
sqrtof = (mpu.acceleration.x)**2 + (mpu.acceleration.y)**2 + (mpu.acceleration.z)**2 #total acceleration for all three axes
totalaccel = math.sqrt(sqrtof) #this is the second part of the formula for the combined acceleration of all three axes
steps = 0 #steps

threshold = 80 #this is the amount of acceleration needed to register as a step
steptrue = 0 #since our code is a while true to award someone a step when they meet the required acceleration,
...
while True: #run forever
    if totalaccel>threshold and steptrue == 0: #When someone takes a step and the bool is false(meaning now it registers a step)
        steps = steps+1 # adds a step
        steptrue = 1 # sets the bool to true 
    elif totalaccel>threshold and steptrue == 1: #they still are over the threshold but have already met the condition before
        pass
    if totalaccel<threshold and steptrue == 1:   #the person was over the threshold but has now dipped under, setting the bool back to false
        steptrue = 0
